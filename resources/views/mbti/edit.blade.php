<x-app-layout>
    <div class="min-h-screen py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-8">
                <h1 class="text-4xl font-bold text-gray-900 mb-4">Edit Tes MBTI Anda</h1>
                <p class="text-lg text-gray-600">Perbarui jawaban Anda untuk tes ini.</p>
            </div>
            ...
            <!-- Quiz Form -->
            <form method="POST" action= "{{ route("mbti.update", $test->id) }}" id="quiz-form">
                @csrf
                @method("PUT")
                <div class="space-y-8">
                        ...
                        <div class="question-card bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-lg {{ $index > 0 ? 'hidden' : '' }}" data-question="{{ $index }}">
                            ...
                            <div class="space-y-4">
                                <label class="flex items-center p-4 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 transition-colors group">
                                    <input type="radio" name="answers[{{ $question->id }}]" value="A" class="sr-only answer-input" data-question="{{ $index }}" {{ (isset($test->answers[$question->id]) && $test->answers[$question->id] == 'A') ? 'checked' : '' }}>
                                    <div class="w-5 h-5 border-2 border-gray-300 rounded-full mr-4 flex items-center justify-center group-hover:border-purple-500 transition-colors">
                                        <div class="w-3 h-3 bg-purple-600 rounded-full {{ (isset($test->answers[$question->id]) && $test->answers[$question->id] == 'A') ? '' : 'hidden' }} radio-dot"></div>
                                    </div>
                                    <span class="text-gray-700 group-hover:text-gray-900 transition-colors">{{ $question->option_a }}</span>
                                </label>

                                <label class="flex items-center p-4 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 transition-colors group">
                                    <input type="radio" name="answers[{{ $question->id }}]" value="B" class="sr-only answer-input" data-question="{{ $index }}" {{ (isset($test->answers[$question->id]) && $test->answers[$question->id] == 'B') ? 'checked' : '' }}>
                                    <div class="w-5 h-5 border-2 border-gray-300 rounded-full mr-4 flex items-center justify-center group-hover:border-purple-500 transition-colors">
                                        <div class="w-3 h-3 bg-purple-600 rounded-full {{ (isset($test->answers[$question->id]) && $test->answers[$question->id] == 'B') ? '' : 'hidden' }} radio-dot"></div>
                                    </div>
    ...
    <script>
        ...
        // Initialize answers from existing test data
        @foreach($test->answers as $questionId => $answer)
        ...
        document.querySelectorAll(".answer-input").forEach(input => {
            ...
            if (answers[questionId] === input.value) {
                input.checked = true;
